...
const EventsMenu = ({ cat }: any) => {
  const category = decodeURIComponent(cat?.cat)
  ...
  const fetchLeagues = (event: string) => {
    if (socket) {
  ...
  useEffect(() => {
    fetchLeagues(cat?.subcat);
  }, [cat?.subcat,socket]);

  useEffect(() => {
    if (category && sportsCategories) {
  console.log(sportsCategories,"Category Event")
      
      setEvents(sportsCategories.find((item) => item.group === category)?.items || []);
    }
