                      ...
                      item?.outcomes?.map(
                        (outcome: any, outcomeIndex: number) => (
                          <button
                            ...
                            key={outcomeIndex}
                            className={`w-full py-2 rounded-lg group relative text-sm disabled:bg-[#27252A] disabled:border-[#4A484D] disabled:cursor-not-allowed transition-colors border-[1px] ${outcome?.point ? "block" : "flex space-x-.5"
                              } items-center justify-between px-1 group ${isBetInAllBets(
                                generateId(
                                  leagues_Info.id,
                                  outcome.name,
                          ...
                          >
                            ...
                            <div
                              className={`flex items-center text-xs justify-between pt-2 ${outcome?.point ? "flex" : "hidden"
                                }  text-red-400`}
                            >
                              <span
                                className={`${outcome?.point < 0
                                    ? "text-red-500"
                                    : "text-green-500"
                                  }`}
                              >
                            ...
                            <div
                              className={`text-xs text-white py-1 ${outcome?.point ? "hidden" : "block"
